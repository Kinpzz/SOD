...
function [P,S,M] = getProposals(I, net, param)
...
M=net.blobs('mask_att_fusion_softmax').get_data();
M=permute(M(:,:,2,1), [2 1 3]);
...
net.forward({Ip});
scores = reshape(net.blobs('prob').get_data(),[],param.batchSize);
